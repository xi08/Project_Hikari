...
#define IS_Timer_Periph(PERIPH)                                                                                          \
    (((PERIPH) == Timer0) || ((PERIPH) == Timer1) || ((PERIPH) == Timer2) || ((PERIPH) == Timer3) || ((PERIPH) == Timer4))
...
#define IS_Timer_Mode(MODE)                                                                                              \
    (((MODE) == Timer_Mode_16AR) || ((MODE) == Timer_Mode_16) || ((MODE) == Timer_Mode_8AR) || ((MODE) == Timer_Mode_16NR))
...
#define IS_Timer_ClkSrc(SRC) (((SRC) == Timer_ClkSrc_DIV1) || ((SRC) == Timer_ClkSrc_DIV12) || ((SRC) == Timer_ClkSrc_EXT))
...
typedef struct
{
    ...
    uint16_t Timer_LoadValue;    // 定时器初始装载值
